...
function comprasCtrl(globales, $scope, $http) {
    ...
    vm.VerComprasPaginado = function () {



        var urlPaginado = globales.urlWS + "ComprasPrVerporPag" + globales.urlCallBack + "&offset=" + globales.offset + "&first=" + globales.first;

        $http.jsonp(urlPaginado)
            .success(function (datap) {
                vm.compras = datap;
                vm.cargando = false;
                console.log("-- VER COMPRAS PAGINADO --");

                console.log("listado paginado de compras:");
